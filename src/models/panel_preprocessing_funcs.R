...
add_vhi_panel_vars <- function(vhi_df) {
  ...
  vhi_df %>% 
    mutate(Canicula_Label =  factor(Canicula_Label, labels = canicula_labels),   # convert canicula index to ordered factor
           munic_dep = paste(Municipio, Departamento, sep = "_"), # make unique identifier for each spatial unit as Municipality_Department
           mean_vhi = mean_vhi * 100) # make vhi on scale from 1-100 to make coefs easier to interpret
}
...
pre_panel_vhi <- function(vhi_df, panel_start_year) {
  ...
  pre_panel_vhi <- vhi_df %>% filter(Year < panel_start_year) %>%
    group_by(shapeID) %>%
    summarise(pre_panel_vhi = mean(mean_vhi))
  
  vhi_df %>% filter(Year >= panel_start_year) %>%
...
build_clean_panel_sf <- function(admin_bndry_sf, vhi_df, hom_df, panel_start_year = 2013) {
  ...
  remove_dup_spatial_units(full_sf)
}
